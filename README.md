...
#### `info = IdentityKey.verify(proof, attestedData, opts ={ epoch, identityPublicKey, devicePublicKey })`

Verify a proof.

Returns `null` if verification fails, otherwise an object with:
- `epoch`: a monotonic counter for which the proof is valid
...
Optionally pass any of `epoch`, `identityPublicKey` or `devicePublicKey`. Verification will fail if:

- proof's epoch is less than any `epoch` provided
